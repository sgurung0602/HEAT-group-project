...
public class OptionsWindow {
  ...
  private void jbInit() throws Exception {
    ...
    JPanel panelFontSizes = new JPanel(new GridLayout(1,1));
    jcbOutputFontSize = new JComboBox();
    jcbCodeFontSize = new JComboBox();
 
    
    JPanel editorFontSize = new JPanel();
    editorFontSize.add(new JLabel("SELECT FONT SIZE:"));
    panelFontSizes.add(editorFontSize);
    
    //create preset buttons for 
    
    JButton buttonSmall = createSmallButton();
    JButton buttonMedium = createMediumButton();
    JButton buttonLarge = createLargeButton();
    editorFontSize.add(buttonSmall);  // Add the button to the panel
    editorFontSize.add(buttonMedium);
    editorFontSize.add(buttonLarge);
    ...
    tabOptions.addTab("Settings", panelFontSizes);
  ...
  public void show() {
    ...
    JTabbedPane tabOptions = (JTabbedPane) panelOptions.getComponent(0); // Assuming the first component is JTabbedPane
    tabOptions.setSelectedIndex(tabOptions.indexOfTab("Settings"));
...
//  }
  ...
  private JButton createSmallButton() {
	    ...
	    buttonSmall.addActionListener(new ActionListener() {
	        public void actionPerformed(ActionEvent e) {
	            // Set font size to a predefined value (e.g., 30)
	            int smallFontSize = 20;
	            WindowManager wm = WindowManager.getInstance();
	            wm.getConsoleWindow().setFontSize(smallFontSize);
	            wm.getEditorWindow().setFontSize(smallFontSize);
	        }
	    });
	    return buttonSmall;
	}
	...
	private JButton createMediumButton() {
	    ...
	    buttonMedium.addActionListener(new ActionListener() {
	        public void actionPerformed(ActionEvent e) {
	            // Set font size to a predefined value (e.g., 50)
	            int mediumFontSize = 35;
	            WindowManager wm = WindowManager.getInstance();
	            wm.getConsoleWindow().setFontSize(mediumFontSize);
	            wm.getEditorWindow().setFontSize(mediumFontSize);
	        }
	    });
	    return buttonMedium;
	}
	...
	private JButton createLargeButton() {
	    ...
	    buttonLarge.addActionListener(new ActionListener() {
	        public void actionPerformed(ActionEvent e) {
	            // Set font size to a predefined value (e.g., 70)
	            int largeFontSize = 50;
	            WindowManager wm = WindowManager.getInstance();
	            wm.getConsoleWindow().setFontSize(largeFontSize);
	            wm.getEditorWindow().setFontSize(largeFontSize);
	        }
	    });
	    return buttonLarge;
	}
